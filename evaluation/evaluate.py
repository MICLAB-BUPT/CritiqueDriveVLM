"""DriveLMM-o1 evaluation: score model outputs with GPT-4o (official protocol).

Uses GPT-4o as the judge to score reasoning-step quality and final-answer / MCQ
correctness, following the DriveLMM-o1 benchmark protocol.

Set your key first:  export OPENAI_API_KEY=sk-...
(Optionally set OPENAI_BASE_URL to use a compatible endpoint.)
...
# ================= CONFIG (edit these) =================
# The API key is read from the OPENAI_API_KEY environment variable (never hardcode it).
client = OpenAI(
...
input_file = "/path/to/results/model_outputs.json"      # inference results to score
output_file = "DriveLMMo1_result.json"                  # per-sample scores
dataset = "/path/to/data/DriveLMMo1_TEST.json"          # DriveLMM-o1 test annotations
...
# Score one reasoning trace with the GPT-4o judge.
...
# 1. The 5 core driving metrics reported in the paper
driving_metrics = [
    "Risk Assessment Accuracy",
    "Traffic Rule Adherence",
    "Scene Awareness & Object Understanding",
...
for metric in driving_metrics:
    if metric in df.columns:
        # Convert to a 0-100 scale
        score = df[metric].mean() * 10
        print(f"{metric:<40} | {score:.2f}")

print("-" * 50)

# 2. Overall reasoning score (mean of the Overall Score column, scaled to 100)
if "Overall Score" in df.columns:
    overall_score = df["Overall Score"].mean() * 10
    print(f"{'Overall Reasoning Score':<40} | {overall_score:.2f}")

# 3. MCQ accuracy
df_mcq = df[df["mcq"] != -1]
if not df_mcq.empty:
    mcq_acc = df_mcq["mcq"].mean() * 100
    print(f"{'Final Answer Accuracy (MCQ)':<40} | {mcq_acc:.2f}%")
else:
    print(f"{'Final Answer Accuracy (MCQ)':<40} | N/A")
